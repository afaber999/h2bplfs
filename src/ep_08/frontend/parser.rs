...
impl Parser {
    ...
    fn expect<'a>(tokens: &'a mut TokenIter, kind: TokenKind, msg: &str) -> &'a Token {
        ...
        let nt = nt.unwrap();
        if nt.kind != kind {
            panic!("{} got {:?} token: {nt:?}", msg, nt.kind);
        }
        nt
    }
        ...
        match next_token.kind {
            ...
            TokenKind::OpenParen => {
                Self::expect(
                    tokens,
                    TokenKind::OpenParen,
                    "Expected open paren in parenterized expression",
                );
                let e = Self::parse_expression(tokens).unwrap();
                Self::expect(
                    tokens,
                    TokenKind::CloseParen,
                    "Expected close paren in parenterized expression",
                );
                Some(e)
            }
            _ => {
    ...
    fn parse_object_expression(tokens: &mut TokenIter) -> Option<AstExpression> {
        if Self::at(tokens).kind != TokenKind::OpenBrace {
            Self::parse_additive_expression(tokens)
        } else {
            let bt = Self::eat(tokens); // eat brace
            println!("PArse object expression {bt:?}");

            let mut properties = Vec::new();

            while Self::at(tokens).kind != TokenKind::Eof
                && Self::at(tokens).kind != TokenKind::CloseBrace
            {
                // three cases to handle
                // { key }
                // { key : val }
                // { key : val, key2 : val2 }
                let id = Self::expect(tokens, TokenKind::Identifier, "Object literal key expected")
                    .value
                    .clone();
                println!("FOUND ID {:?}", &id);

                if Self::at(tokens).kind == TokenKind::Comma {
                    // shorthand key with comma {key,}
                    Self::eat(tokens); // eat comma
                    properties.push(AstProperty {
                        key: id,
                        value: None,
                    });
                    continue;
                } else if Self::at(tokens).kind == TokenKind::CloseBrace {
                    // shorthand keu without comma {key}
                    properties.push(AstProperty {
                        key: id,
                        value: None,
                    });
                ...
                Self::expect(
                    tokens,
                    TokenKind::Colon,
                    "Missing colon following identifier",
                );

                println!("GOT COLON PARSING EXPRE ");
                let value = Self::parse_assignment_expression(tokens).unwrap();
                println!("GOT EXPRE {:?} ", value.clone());
                ...
                // When next token is not a closing brace, expect a comma
                if Self::at(tokens).kind != TokenKind::CloseBrace {
                    Self::expect(
                        tokens,
                        TokenKind::Comma,
                        "Expect comma or closing brace following property",
                    );
                }
            }
            ...
            Self::expect(
                ...
                "Expect object missing closing brace",
            );

            Some(AstExpression::ObjectLiteral(AstObjectLiteral {
                properties,
            }))
        }
    ...
    fn parse_assignment_expression(tokens: &mut TokenIter) -> Option<AstExpression> {
        let left = Self::parse_object_expression(tokens).unwrap();
    ...
    fn parse_var_declartion(tokens: &mut TokenIter) -> Option<AstStatement> {
        ...
        let identifier = Self::expect(
            tokens,
            TokenKind::Identifier,
            "Expected identfier in var delcartion",
        )
        .value
        .clone();
        ...
        if Self::at(tokens).kind == TokenKind::SemiColon {
            ...
            let _equals = Self::expect(
                tokens,
                TokenKind::Equals,
                "Expected equals after identfier in var delcartion",
            );
            let value = Self::parse_expression(tokens);
            let _semi = Self::expect(
                tokens,
                TokenKind::SemiColon,
                "Expected semicolon after var/const delcartion",
            );
